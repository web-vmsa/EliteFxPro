...
<body>	
			...
			echo "
				<div title='Click this message to close the warning.' onclick='fecharerror();' id='error-login' class='error-login'>
					...
					<div class='right-error-login-text background-dark-green'>
						<h2><r>PROFIT UPDATE !</r></h2>
						<p style='color: lightgrey;'>You have successfully updated user earnings !</p>
					</div>
				</div>
			";
		}
	...
	<div class="table-user">
		...
		<div class="title-description">
			<h2>Update Profit</h2>
			<p>Update your users' earnings above</p>
		</div>
	</div>
	<!-- Table users -->
	...
	<div class="search-user">
		<form method="GET">
			<input type="text" name="search" placeholder="Find a user"> <button>SEARCH</button>
		</form>
	</div>
	<!-- Search -->
	...
	<div class="fake-table">
		<table>
			<tr>
				...
				<th><img src="images/overflow.png"></th>
				<th><img src="images/interactive.png"></th>
			</tr>
			<?php
					$nome = $_GET['search'];
					$novo = new Adm();
					$novolista = $novo->SelectAUser($nome);
					foreach($novolista as $dadosnovos):
			?>
			<tr>
				<td><?php echo $dadosnovos['name']; ?></td>
				<td><?php echo $dadosnovos['email']; ?></td>
				<td><?php echo $dadosnovos['has']; ?></td>
				<form method="POST">
					<td>Add <input type="text" name="quantity"></td>
